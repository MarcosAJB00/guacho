...
!> @brief ppv_utilities
!> @details Utilities to compute PPV data cubes in the optically thin
!> limit

module ppv_utilities
...
contains
  ...
  !initializes MPI
    ...
    print '(a)' ,'*********************************************'
#endif
    if(rank.eq.master) print '(a)', 'Calculating Position Position Velocity data'
    ...
    take_turns : do ip=0,np-1
      if (rank == ip) then
        ...
        write(file1,'(a,i3.3,a,i3.3,a)')                                       &
              trim(filepath)//'points',rank,'.',itprint,'.bin'
        unitin=rank+10
#else
        write(file1,'(a,i3.3,a)') trim(filepath)//'points',itprint,'.bin'
        unitin=10
#endif
  ...
  subroutine fill_map(nxmap,nymap,nvmap,vmin,vmax,u,map,dxT,dyT,&
    ...
    use parameters, only : nxmin, nxmax, nymin, nymax, nzmin, nzmax,           &
                         neq, nx, ny, nz, vsc, rsc, nztot, neqdyn
    use globals, only : dz
    ...
    integer :: i,j,k, iobs, jobs, kobs
    real :: x,y,z,xn,yn,zn, vx, vy, vz,vxn, vyn, vzn, velsc
    real :: T, prim(neq), deltaV

    deltaV = ( vmax - vmin ) / nvmap
      ...
      do j=1,ny
        do i=1,nx
          ...
          if( (iobs >=1    ).and.(jobs >=1    ).and. &
              ...
              !kobs es la posicion en V de v_LOS = vzn
              kobs = ( vzn - vmin ) / deltaV
              kobs = max(kobs,1)
              kobs = min(kobs,nvmap)
              map(iobs,jobs,kobs)= map(iobs,jobs,kobs) + prim(1)
          end if
        end do
      end do
    end do
  ...
  subroutine  write_ppv(itprint,filepath,nxmap,nymap,nvmap,map,degx,degy,degz)

    implicit none
    integer, intent(in) :: nxmap, nymap,nvmap,itprint
    real, intent(in)    :: map(nxmap,nymap,nvmap)
    integer             ::  unitout
    integer, intent(in) :: degx,degy,degz
    character (len=128), intent(in) :: filepath
    character (len=128) file1

     
    write(file1,'(a,i2.2,a,i2.2,a,i2.2,a,i3.3,a)')  trim(filepath)//'ppv_',degx,'_',degy,'_',degz,'-',itprint,'.bin'
    unitout=11
  ...
  end subroutine write_ppv

  !=======================================================================
end module ppv_utilities

!=======================================================================
!> @brief Computes PPV data cube
!> @details Computes Position Position Velocity data cubes
...
program ppv
...
#endif
  implicit none
  character (len=128) :: filepath, filepath_in
  integer :: err
  integer :: itprint
  integer, parameter :: degx = 80, degy = 0, degz = 0
  real, parameter :: theta_x = degx *deg
  real, parameter :: theta_y = degy *deg
  real, parameter :: theta_z = degz *deg
  !   map and its dimensions
  integer, parameter :: nvmap= 200
  integer            :: nxmap, nymap
  real :: dxT, dyT, vmin,vmax
  real, allocatable :: map(:,:,:), map1(:,:,:)

  nxmap = nxtot
  nymap = nytot*3.   ! hacked for jet/core simulation
  ...
  !  hack to include different input path
  filepath_in = trim('/storage3/esquivel/Core5/BIN_2b/')

  ! choose output (add option later to input form screen)
  !filepath=trim(outputpath)
  filepath=trim('/storage3/esquivel/Core5/BIN_2b/')

  loop_over_outputs : do itprint=41,80      ! Change output to analize

    !  read ph and u from file
    call read_data(u,itprint,filepath_in)
    ...
    !
    if (rank == master) then
       print'(a)', 'Calculating PPV with angles of rotaton'
    ...
    !  write result
    if (rank == master) then
      call write_ppv(itprint,filepath,nxmap,nymap,nvmap,map1,degx,degy,degz)
    end if
