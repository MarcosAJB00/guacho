!=================================
 ...
 module network
   ...
   ! number of species
   integer, parameter :: n_spec = 6
   ! HII and HeII fraction and pop fraction between singlet and triplet state in he
   !real (kind=8), parameter :: fHII =1.7e-3, fHeII=1.9e-9,! alpha=1.9e-11
...
subroutine gsub(y, q, d, time, T, phHI, phHeIS, phHeIM)
    ...
    implicit none
    real , intent(in)  :: y(n_spec)
    real , intent(out) :: q(n_spec)     ! Producción
    real , intent(out) :: d(n_spec)     ! Tasa de pérdida
    real , intent(in)  :: time
    real (kind=8), intent(in)  :: T, phHI, phHeIS, phHeIM
