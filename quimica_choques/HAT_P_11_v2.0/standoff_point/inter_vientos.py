...
print(f"Radio sónica: {r_sonic/Rp_cm:.3f} Rp")
...
plt.plot(rp/Rp_cm, cs_p/1e5, label='Velocidad del sonido')
plt.scatter(r_sonic/Rp_cm, v_p[np.argmin(np.abs(v_p - cs_p))]/1e5, color='red', zorder=5, label=f'Punto sónica = {r_sonic/Rp_cm:.3f} Rp')
plt.yscale('log')
plt.xlabel('Distancia desde el planeta [R_p]')
plt.ylabel('Velocidad [Km/s]')
...
r_sonic_idx = np.argmin(np.abs(r_grid - r_sonic))
ax.scatter(r_sonic/Rp_cm, P_p_grid[r_sonic_idx], color='red', zorder=5, label=f'Punto sónico = {r_sonic/Rp_cm:.3f} Rp')
