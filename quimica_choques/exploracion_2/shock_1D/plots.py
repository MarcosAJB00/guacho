...
for i in range(len(model_numbers)):
    ...
    #if T0_values[i] == 1000.0:
    #    ax_rho.plot(x, rho/mp, linestyle='--', alpha=0.6)
    #    ax_T.plot(x, T, linestyle='--', alpha=0.6)
    #elif T0_values[i] == 1500.0:
    #    ax_rho.plot(x, rho/mp, linestyle=':', label=f'Model {int(model_numbers[i])}')
    #    ax_T.plot(x, T, linestyle=':', label=f'Model {int(model_numbers[i])}')
    #else:
    #    ax_rho.plot(x, rho/mp, linestyle='-', label=f'Model {int(model_numbers[i])}')
    #    ax_T.plot(x, T, linestyle='-', label=f'Model {int(model_numbers[i])}')

    if model_numbers[i] != 40:
        ax_rho.plot(x, rho/mp, linestyle='--', alpha=0.6)
        ax_T.plot(x, T, linestyle='--', alpha=0.6)
    else:
        ax_rho.plot(x, rho/mp, lw=3, linestyle='-', color='red', label=f'Model {int(model_numbers[i])}')
        ax_T.plot(x, T, lw=3, linestyle='-', color='red', label=f'Model {int(model_numbers[i])}')
