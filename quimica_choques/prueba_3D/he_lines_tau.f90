...
!> @details Cada archivo es un dump binario crudo de nx*ny*nz reales,
...
subroutine read_data_prim(dens_he, temp, vmod, filepath)
  ...
  implicit none
  real, intent(out) :: dens_he(nx,ny,nz), temp(nx,ny,nz), vmod(nx,ny,nz)
...
subroutine fill_map_prim(nxmap,nymap,nvmap,vmin,vmax,dens_he,temp,vmod,map_he,&
  ...
  use parameters, only : nx, ny, nz, vsc, rsc
  use globals,    only : dz
  ...
  real,    intent(in) :: dens_he(nx,ny,nz), temp(nx,ny,nz), vmod(nx,ny,nz)
  ...
  real    :: xp,yp,zp, rad
  real    :: prof_3pj0(nvmap),prof_3pj1(nvmap), prof_3pj2(nvmap), cte
        ...
        do i=1,nx
          ...
          if( (iobs >= 1    ).and.(jobs >= 1    ).and. &
            ...
            if(zn > 0) then
              ...
              !  corrección por corrotación (marco inercial)
              vx = vx + omegap*z
              vz = vz - omegap*x
...
program helines_tau_map
  ...
  real, parameter    :: thetay_max=10, thetay_min=-10
  integer            :: i
  ...
  real, allocatable  :: dens_he(:,:,:), temp(:,:,:), vmod(:,:,:)
  real, allocatable  :: map_he(:,:,:)
