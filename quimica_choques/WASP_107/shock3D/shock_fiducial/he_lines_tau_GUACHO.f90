...
use parameters
use globals, only : u, dx, dy, dz, coords, rank, comm3d!, left, right, &
                    !top, bottom, out, in
...
implicit none
  ...
  if(rank.eq.master) then
     ...
     print '(a)' ,'*******************************************'
     print '(a)', 'Calculating helium lines tau map'
  end if
  ...
  print '(a)' ,'*******************************************'
  print '(a)', 'Calculating Helium Tau Map'
#endif
...
subroutine fill_map(nxmap,nymap,nvmap,vmin,vmax,u,map_he,dxT,dyT,&
                   theta_x,theta_y,theta_z)!,n2_save)
  ...
  real,   intent(out) :: map_he(nxmap,nymap,nvmap)
  integer :: i,j,k, iobs, jobs
  real    :: x,y,z,xn,yn,zn, vx, vy, vz,vxn, vyn, vzn
  real    :: T, prim(neq), prof_3pj0(nvmap),prof_3pj1(nvmap),prof_3pj2(nvmap),cte

  !transition from 2 3S to 2 3P for J=0,1,2
  real, parameter :: f3pj0 = 5.9902e-2, lambda3pj0 = 1082.909e-7, Aij= 1.0216e7 !1/s
  real, parameter :: f3pj1 = 1.7974e-1, lambda3pj1 = 1083.025e-7 !cm
  real, parameter :: f3pj2 = 2.9958e-1, lambda3pj2 = 1083.034e-7
  real            :: sigma3pj0, sigma3pj1, sigma3pj2
  real            :: shift_1, shift_2
          ...
          if( (iobs >= 1    ).and.(jobs >= 1    ).and. &
            ...
            if(zn > 0) then
             ...
             !if (prim(neqdyn+2)<0) then
              !call phivoigt(T,-vzn,vmin,vmax,nvmap,prof_3pj0,lambda3pj0,Aij,0.0)!shift_1)
              !call phivoigt(T,-vzn,vmin,vmax,nvmap,prof_3pj1,lambda3pj1,Aij,0.0)!shift_2)
              call phivoigt(T,-vzn,vmin,vmax,nvmap,prof_3pj2,lambda3pj2,Aij,0.0    )

              map_he(iobs,jobs,:) = map_he(iobs,jobs,:) + dz*rsc*prim(neqdyn+4)*&
...
subroutine  write_maps(itprint,i,filepath,nxmap,nymap,nvmap,map_he)

  use parameters, only: nxtot,nytot,nztot

  implicit none
  integer, intent(in) :: nxmap, nymap,nvmap,itprint,i
  real, intent(in)    :: map_he(nxmap,nymap,nvmap)
  character (len=128), intent(in) :: filepath
  character (len=128) file1
  integer ::  unitout

  write(file1,'(a,i3.3,a,i3.3,a)')  trim(filepath)//'helines/HE_tau_3pj2_',itprint,'_',i,'.bin'
  unitout=11
...
subroutine phivoigt(T,vzn,vmin,vmax,nvmap,profile, wvl, Ga, shift)
  use constants, only: amh, pi, kB, clight
  implicit none
  real, intent(in)    :: T, vzn,vmin, vmax, wvl, Ga,shift
  real, intent(out)   :: profile(nvmap)
  integer, intent(in) :: nvmap
  integer :: i
  real    :: sigm,gamm,dv,vr,a,v,coef!ccs,a0,
  complex (kind = 8) :: W
...
program helines_tau_map
  ...
  allocate(map_he(nxmap,nymap,nvmap) ,map1(nxmap,nymap,nvmap))
  ...
  !filepath = trim(outputpath)   !//'WindRot/'
  filepath = 'WindRot/'

  loop_outputs : do itprint = 100, 120, 5
    ...
    loop_over_theta_y : do i=0,nsteps
      ...
      !  add info to the map
      call fill_map(nxmap,nymap,nvmap,vmin,vmax,u,map_he,dxT,dyT,theta_x,theta_y,theta_z)

      !  sum all the partial sums
       call mpi_reduce(map_he,map1,nxmap*nymap*nvmap, mpi_real_kind, mpi_sum, master, comm3d, err)

      !  write result
      if (rank == master) then
        call write_maps(itprint,i,filepath,nxmap,nymap,nvmap,map1)
      end if
