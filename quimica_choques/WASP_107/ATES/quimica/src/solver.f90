...
contains
  ...
  subroutine chemeq2solve(dtg, y, n, iT, iphHI, iphHeIS, iphHeIM)
    ...
    implicit none
    integer, intent(in)    :: n
    real(kind=8),    intent(in)    :: dtg
    real(kind=8),    intent(inout) :: y(n)
    integer :: i, gcount=0, rcount=0 !inicializados por mi
    integer :: iter
    real(kind=8)  :: ts, tn, tfd, ne
    real(kind=8)  :: q(n), d(n), rtaus(n), y1(n)
    real(kind=8)  :: alpha, qs(n)
    real(kind=8)  :: ys(n), y0(n), rtau(n)
    real(kind=8)  :: scr1, scr2, scrarray(n)
    real(kind=8)  :: dt, dto
!    real    :: rswitch  !no se usa aca
    real(kind=8)    :: scrtch, ascr, eps
    ...
    !write(filename, '(A,I0,A)') './output/salida_', j_index, '.txt'
...
!    end if
  ...
  subroutine chemsp(epsmn, epsmx, dtmn, tnot, itermx, ns, ymn, prt)
    ...
    implicit none
    real(kind=8),    intent(in)    :: epsmn, epsmx, dtmn, tnot, prt
    integer, intent(in)    :: ns
    real(kind=8),    intent(inout) :: ymn(ns)
  ...
  subroutine chemer(y, n, dt, tn, dtmin, epsmin, q, d, rtau, ys, y0, ymin)
    ...
    implicit none
    integer, intent(in) :: n
    real(kind=8), intent(in)    :: y(n), q(n), d(n), rtau(n), ys(n), y0(n), ymin(n)
    real(kind=8), intent(in)    :: dt, tn, dtmin, epsmin
    integer :: i
    real(kind=8) :: dtc
