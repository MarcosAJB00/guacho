...
f0 = FortranFile('helines/HE_tau_3pj0_001.bin', 'r')
tau0 = f0.read_reals(np.float64)
tau0 = tau0.reshape((nx, ny, nv), order='F')

f1 = FortranFile('helines/HE_tau_3pj1_001.bin', 'r')
tau1 = f1.read_reals(np.float64)
tau1 = tau1.reshape((nx, ny, nv), order='F')

f2 = FortranFile('helines/HE_tau_3pj2_001.bin', 'r')
...
ax[0].imshow(
    ...
    origin='lower',
    cmap='inferno',
    norm=LogNorm(vmin=1e-2, vmax=1e6)
)
ax[0].set_title('3P0')
...
#plt.tight_layout()
plt.savefig('tau_maps_central.png', dpi=300,bbox_inches='tight')
plt.close()
