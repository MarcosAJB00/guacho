...
contains
  ...
  subroutine u2prim(uu, prim, T)
    ...
    use constants
    implicit none
    real,    intent(inout), dimension(neq)  :: uu
    ...
    if (eq_of_state == EOS_CHEM) then
      ...
      !  DEBUGIG NEGATIVE PRESSURES
      !  make sure floors propagate to conserved variables
      uu(1) = prim(1)
      !  Total energy (no B field --HD and passive MHD--)
