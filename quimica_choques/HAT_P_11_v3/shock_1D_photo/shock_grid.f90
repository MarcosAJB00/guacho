program shock_grid 
  ...
  real(kind=8), parameter :: T_corte = 7.0d2 ! K, temperatura de corte para terminar el modelo
  real(kind=8), parameter :: Eheat = 2.243047d-12 ! erg, (15-13.6) ev
  real(kind=8), parameter :: FluxHI = 1.337d+14 ! cm^-2 s^-1, flujo de fotones ionizantes de H
...
!4.35136e+00 8.00926e+06 1.76859e+03 9.27221e+05
  ...
  n0_grid = (/ 1.23921d+07 /) ! cm^-3
  T0_grid = (/ 2.21866d+03 /) ! K
  u0_grid = (/ 9.17809d+05 /) ! cm/s
  ...
  do i = 1, T0_points
    ...
    call run_model_initial(y0_grid(1), n0_grid(i), u0_grid(i), T0_grid(i), n_model)
...
!=======================================================================
subroutine coef_phi0(phi) ! Coeficiente de fotoionizacion del H
  implicit none
...
end subroutine coef_phi0
!=======================================================================


!=======================================================================
subroutine run_model_initial(y0i, n0i, u0i, T0i, model_number)
  ...
  !real(kind=8), intent(in) :: phiH
  
  real(kind=8) :: y, n, u, T
  real(kind=8) :: rho, P, h, x, dx, G, L
  real(kind=8) :: rho0, u0, P0, n0, ne
  real(kind=8) :: c, alpha, phiH, dhdx, dydx
  ...
  write(20,'(A)') '# x[cm]  rho[g/cm^3]   T[K]    y    n[cm^-3]    u[cm/s]    P[dyn/cm^2]    h[erg/g]    L[dyn/cm^3]   G[dyn/cm^3]'
  write(20,*) x, rho, T0i, y, n, u, P, h, L, G
  ...
  do while (contador < 2)
    ...
    dhdx = (G - L) / (rho*u)
    dydx = ((n*y)*((1.0d0 - y)*c - y*alpha) + (1.0d0 - y)*phiH)/u
    ...
    h = h + dhdx*dx
    y = y + dydx*dx
    y = max(0.d0, min(1.d0, y)) !asegurar que y este entre 0 y 1
    ...
    write(20,*) x, rho, T, y, n, u, P, h, L, G
...
end subroutine run_model_initial
