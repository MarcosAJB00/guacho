...
module parameters
  ...
  !! Also in user_mod the boundaries for sources (e.g. winds/outflows)
  !! are set
  ...
  integer, parameter :: bc_out    = BC_OUTFLOW !!if SW is active then BC_OTHER !( out es z=0)
  ...
  !>  LIMITER_VAN_LEER   = Van Le limiter
  ...
  integer, parameter :: n1_chem = 6     !< position of 1st index of  chem spec.
#else
  ...
  integer, parameter :: MPI_NBX=1     !< number of MPI blocks in X0º
  ...
  real, parameter :: xphys=2.0*5.09421794574017*(0.4466*Rjup) !< grid extent in X (physical units, cgs)
  ...
  real, parameter :: T0=1.e3                !<  reference temperature (for cs)
  ...
  real, parameter :: dtprint = 0.03*day/tsc
  ...
  integer            :: itprint0 = 47        !< number of output to do warm start
