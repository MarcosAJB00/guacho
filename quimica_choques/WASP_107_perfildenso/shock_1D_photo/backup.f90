program shock_grid 
  ...
  u0_grid = (/ 10.0d0*1.0d5, 50.0d0*1.0d5, 100.0d0*1.0d5, 200.0d0*1.0d5 /)
  y0_grid = (/ 1.0d-4, 1.0d-3, 1.0d-2, 1.0d-1 /) ! fraccion de ionizacion inicial
  
  n_model = 0

  open(10, file='model_list.dat', status='unknown', action='write')
    ...
    do j = 1, n0_points
      do k = 1, u0_points
        do z = 1, y0_points
          ...
          print *, 'Running model ', n_model, 'T0 = ', T0_grid(i), ', n0 = ', n0_grid(j), &
                   ', u0 = ', u0_grid(k), ', y0 = ', y0_grid(z)
          write(10,*) 'model' // trim(n_model), T0_grid(i), n0_grid(j), u0_grid(k), y0_grid(z)
          
          !corro el modelo con las condiciones iniciales dadas y guardo los resultados
          call run_model(y0_grid(z), n0_grid(j), u0_grid(k), T0_grid(i), &
           'model' // trim(n_model) )
...
!=======================================================================
subroutine run_model(y, n, u, T, model_name)
  ...
  ! inputs: 
  ! - y: fraccion de ionizacion de H
  ! - n: densidad de particulas (cm^-3)
  ! - u: velocidad del fluido (cm/s)
  ! - T: temperatura (K)
  ! - model_name: nombre del modelo a correr (string)
  ! outputs: perfil de T y rho.

  implicit none

  real(kind=8), intent(in) :: y, n, u, T
  ...
  open(20, file='output' // trim(model_name) // '.dat', status='unknown', action='write')
