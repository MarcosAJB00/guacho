...
contains
      ...
      do j=1,ny
        do i=1,nx
          ...
          if (user_source_terms     .or. &
            ...
            !  Notice -sign in front of dt, changes the sign
  ...
  subroutine tstep()
    use parameters, only : tsc, riemann_solver, eq_of_state, dif_rad, cooling, &
                           th_cond, master, chem_solver
    use constants
    use globals
    use hydro_core,   only : calcprim
    use exoplanet,    only : get_central_mass, output_mdot
#ifdef PASSIVES
    use hrate,        only : update_neutral_fraction
#endif
    use boundaries
    use chemistry,    only : update_chem
    use chemeq2,      only : update_chemeq2
    use cooling_H
    use cooling_DMC
    use cooling_CHI
    ...
    use thermal_cond
    implicit none
    real :: dtm, MiniLoc, MfinLoc, Mini, Mfin
    integer :: err

        !  calcular Masa de objeto central antes de paso de tiempo
    !  Solo cada 500 iteraciones
    if (mod(currentIteration,500)==0) then
      call get_central_mass(u,MiniLoc)
      !  suma la masade c/core en Mini
      call mpi_reduce(MiniLoc ,Mini, 1, mpi_real_kind, mpi_sum, master,comm3d, err)
    end if
    ...
    case(COOL_CHEM)
      ! Chemistry network cooling (primitives in physical domain are updated in
      ! update_chem)
      call chem_cool()
    ...
    if (mod(currentIteration,500)==0) then
      !  calcular Masa de objeto central despues de paso de tiempo
      call get_central_mass(u,MfinLoc)
      !  suma la masade c/core en Mini
      call mpi_reduce(MfinLoc ,Mfin, 1, mpi_real_kind, mpi_sum, master,comm3d, err)
    end if
    ...
    !  Write mass acretion/loss (positive/negative, respectively) rate
    if (rank == master .and. mod(currentIteration,500)==0) then
      call output_mdot(Mini, Mfin, dt_CFL)
    end if
