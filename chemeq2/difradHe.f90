...
module difradHe
  ...
  use exoplanet, only: star
  !use constants, only: Rsun
  use parameters
  use globals

  implicit none
  integer, parameter :: nrays=nx*ny     !< Number of rays
  !> Fotoionization cross sections of HI, HeI anh HeII, and the corresponding S*
  real               :: a0(3), S_ion(3)
  real, allocatable  :: phHI(:,:,:)    !< Photoionizing rate of H I
  real, allocatable  :: phHeIS(:,:,:)  !< Photoionizing rate of HeI (singlet)
  real, allocatable  :: phHeIM(:,:,:)  !< Photoionizing rate of HeI (triplet)
...
contains
  ...
  subroutine init_difradHe()
    ...
    a0(1) = 3.87e-18     ! cross section for H @ lambda = 772.8 A
    a0(2) = 4.03e-18     ! cross section for He ground @ lambda = 338.6 A
    a0(3) = 3.59e-18     ! cross section for He triplet @ lambda = 1717.7 A

    S_ion(1) = star%S0H   !soft-EUV
    S_ion(2) = star%S0He1 !hard-EUV
    S_ion(3) = star%S0He3 !mid-UV

    allocate( phHI   (nx,ny,nz) )
    allocate( phHeIS (nx,ny,nz) )
    allocate( phHeIM (nx,ny,nz) )
  ...
  subroutine photons(xl0,yl0,zl0,xd,yd,zd,f,phi,i_spec)
    ...
    use network, only : iHI, iHeIS, iHeIM
    ...
    case(1)
      i_eq = n1_chem-1+iHI   !n1_chem=6, iH1=1, i_eq=6
    case(2)
      i_eq = n1_chem-1+iHeIS !iHeIS = 3, i_eq= 8
    case(3)
      i_eq = n1_chem-1+iHeIM !iHeIM = 4, i_eq = 9
    end select
 ...
 subroutine radbounds(phi, i_spec)
  ...
  subroutine diffuse_radHe()
    ...
    implicit none
    real :: f,f1!,dirx,diry,dirz
    !real :: xc,yc,zc, xp, yp, zp,srad
    integer :: i,j, err!, ii, jj, kk ,k, niter
...
!    srad=Rstar
    ...
    do i_spec = 1, 3 !  1:HI, 2:HeIS, 3:HeIM
      ...
      if(coords(2) == 0)then
          ...
          !    f1 = 1.0/(1.0 + exp(12.0*(1.0 - time/DT)))
          !    if(rank == 0)then
          !       print'(a,es12.3)',"Photoionization dimmer: ",f1
          !    endif
          ! endif
          ...
          do j = 1,ny
            do i = 1,nx
              in = in + 1
              !S_ion en unidades de  cm^-2 s^-1
              f = f1 *S_ion(i_spec) * dx*rsc * dy*rsc !(3.2679E31)
              call photons(float(i)-0.5, &
                           float(j)-0.5, &
                           0.5, &
                           0.0,0.0,1.0,f,phi,i_spec)
            enddo
          enddo
      endif
!!      do niter=1, nrays
...
!!      end do
    ...
    ! trace photons across boundaries)
      ...
      case(2)  ! HeIS
        phHeIS(:,:,:)   = phi(:,:,:)  ! / real(nmax)

      case(3)  ! HeIM
        phHeIM(:,:,:)   = phi(:,:,:)  ! / real(nmax)
