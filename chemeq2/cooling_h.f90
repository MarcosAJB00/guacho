...
contains
  ...
  subroutine coolingh()
    ...
    use parameters, only : neq, nx, ny, nz, tsc, dif_radHe, charge_exchange
...
!    real (kind=8) :: SHP
  ...
  subroutine  cooling_h_neq(pp, uu, dt, radphi)
    ...
    use parameters, only : neqdyn, dif_radHe, mhd, cv, neq, Tempsc, eq_of_state
    use constants
  ...
  !  use globals, only: rank
    ...
    if (dif_radHe) then
      gain=real(radphi,8)*dh0*3.8e-12 !(16-13.6)*1.60218e-12
    ...
    pp(5) = max(pp(5),(2*pp(1)-pp(neqdyn+1))*10./Tempsc)
